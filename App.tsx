...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen selection:bg-black selection:text-white bg-gray-100">
      ...
      {/* ── Hanging Home Tab — top-right corner near Hero ── */}
      <div className="fixed top-0 right-8 z-[2] group hidden md:block">
        <button
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="flex flex-col items-center gap-1 bg-black text-white px-5 pt-3 pb-4 rounded-b-[1.5rem] shadow-xl hover:pb-6 transition-all duration-300 neo-shadow"
          title="Back to top"
        >
          ...
          <span className="text-[8px] font-black uppercase tracking-widest text-white/60 group-hover:text-white transition-colors leading-none">Top</span>
        </button>
      </div>

      {/* ── Hanging Social Tab — top-right corner near Hero ── */}
      <div className="fixed top-0 right-28 z-[2] group hidden md:block">
        <a
          href={`https://github.com/Meera2906`}
          target="_blank"
          className="flex flex-col items-center gap-1 bg-black text-white px-5 pt-3 pb-4 rounded-b-[1.5rem] shadow-xl hover:pb-6 transition-all duration-300 neo-shadow"
          title="GitHub"
        >
          ...
          <span className="text-[8px] font-black uppercase tracking-widest text-white/60 group-hover:text-white transition-colors leading-none">GitHub</span>
        </a>
      </div>

      <Navigation activeTab={activeTab} setActiveTab={setActiveTab} />
      
      <main className="transition-all duration-500 relative z-[1]">
        <div id="home">
          <Hero />
        </div>
